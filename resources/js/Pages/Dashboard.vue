                            ...
                            <li v-if="transactions.length > 0" v-for="transaction in transactions" :key='transaction.id'
                                ...
                                <div class="flex items-center space-x-4">
                                    <div class="flex-shrink-0">
                                        ...
                                        <svg
                                            v-if="transaction.description == 2 && transaction.wallet_id == $page.props.auth?.user?.wallet?.id"
                                            class="w-5 h-5 text-gray-400 dark:text-white"
                                            fill="none"
                                    ...
                                    <div class="flex-1 min-w-0"
                                        ...
                                        <p class="text-sm font-medium text-gray-900 truncate dark:text-white">
                                            {{
                                                transaction.description == 1 ? 'Depósito' : (transaction.wallet_id == $page.props.auth?.user?.wallet?.id ? 'Tranferência Recebida' : 'Transferência Feita')
                                            }}
                                            <Link
                                                v-if=" transaction.description == 2 && transaction.wallet_id == $page.props.auth?.user?.wallet?.id && !transaction.refunded_at"
                                        ...
                                        <p v-if="transaction.description == 2"
                                           class="text-sm text-gray-500 truncate dark:text-gray-400">
                                            {{
                                                transaction.wallet_id == $page.props.auth?.user?.wallet?.id ? `De: ${transaction.reference?.user?.name}` : `Para: ${transaction.wallet?.user?.name}`
                                            }}
                                        </p>
                                    </div>
